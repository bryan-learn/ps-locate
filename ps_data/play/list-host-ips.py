...
#pull host-name array from each host record. The host-name value can be a name or IP address - ignore the IPs.
for res in response:
    ...
    for host in res['host-name']:
        if host != '':
            ...
            #only print host if it is an ip address
            if v4_match is not None and v6_match is not None:
                print host
