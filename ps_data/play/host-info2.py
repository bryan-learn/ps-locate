...
for res in response:
    #print get_host_info_json(res['host-name'][0])
    for host in res['host-name']:
        if host != '':
            #matchObj = re.match( r'^((25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$', host )
            v4_match = re.match( ipv4_pattern, host)
            v6_match = re.match( ipv6_pattern, host )
            #only print host if not an ip address
            if v4_match is None and v6_match is None:
                print get_host_info_json(host)
            #print get_host_info_json(host)
